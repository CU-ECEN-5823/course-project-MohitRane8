...
//      break;
      ...
      int16_to_buf(&msg_buf[msg_off], req->pb1_press_release);
      msg_off += 2;
      *msg_used = msg_off;
      break;
  }
...
//      break;
      ...
      req->kind = kind;
      req->pb1_press_release = int16_from_buf(&msg_buf[msg_off]);
      break;
  }
...
//        break;
		  ...
		  int16_to_buf(&msg_buf[msg_off], current->pb1_press_release);
		  msg_off += 2;
		  if (target) {
			int16_to_buf(&msg_buf[msg_off], target->pb1_press_release);
			msg_off += 2;
		  }
		  *msg_used = msg_off;
		  break;
...
//        break;
		  ...
		  if (msg_len - msg_off == 2) {
			current->kind = kind;
			current->pb1_press_release = int16_from_buf(&msg_buf[msg_off]);
			msg_off += 2;
			*has_target = 0;
		  } else if (msg_len - msg_off == 4) {
			current->kind = kind;
			...
			msg_off += 2;
			target->kind = kind;
			target->pb1_press_release = int16_from_buf(&msg_buf[msg_off]);
			msg_off += 2;
			*has_target = 1;
		  } else {
			return -1;
